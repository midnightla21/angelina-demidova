...
function Heading({ start, end }: { start: string; end: string }) { return <h2><Lines text={start} /><br /><span className="muted"><Lines text={end} /></span></h2>; }
...
export default function PhysicianSite({ locale, t }: { locale: Locale; t: Dictionary }) {
  ...
  return (
    <main className={`locale-${locale}${paused ? ' motion-paused' : ''}`}>
      ...
      <section className="section approach" id="approach">
        ...
        <div className="two-columns"><Heading start={t.approachHeading} end={t.approachHeadingMuted} /><div className="prose"><p className="lead">{t.approachLead}</p><p>{t.approachText}</p><p className="core">{t.approachCore}</p></div></div>
        <ol className="care-steps">{steps.map(([title, text], i) => <li key={title}><span className="step-number">0{i + 1}</span><h3>{title}</h3><p>{text}</p></li>)}</ol>
      </section>
      ...
      <section className="section recovery" id="recovery">
        ...
        <div className="two-columns"><Heading start={t.recoveryHeading} end={t.recoveryHeadingMuted} /><div className="prose"><p className="lead">{t.recoveryLead}</p><p>{t.recoveryText1}</p><p>{t.recoveryText2}</p></div></div>
      ...
      <section className="section voices" id="voices">
        ...
        <div className="quotes"><figure><blockquote>{t.quote1}</blockquote><figcaption><span>{t.quote1By}</span><External href={sm}>{t.quote1Source}</External></figcaption></figure><figure><blockquote>{t.quote2}</blockquote><figcaption><span>{t.quote2By}</span><External href={reviews}>{t.quote2Source}</External></figcaption></figure></div>
